...
// REFACTORED: Removed "Provider" from the data class name
...
@riverpod
Future<OverallBudgetSummary> overallBudgetSummary(
  OverallBudgetSummaryRef ref,
) async {
  // Get all categories to sum up their absolute ceilings
  ...
  // Get all recurring incomes to calculate total baseline income
  ...
  final totalIncome = recurringIncomes.fold<double>(0.0, (sum, income) {
    switch (income.recurrence) {
      ...
      case RecurrencePeriod.weekly:
        return sum + (income.amount * 4.333); // Slightly higher precision
