...
import 'package:budgit/src/features/budget_hub/presentation/widgets/category_ring_painter.dart';

// FIX: Point to the consolidated weekly providers file
...
class DailySpendingGauges extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // FIX: Watch the renamed shared date
    final selectedDate = ref.watch(weeklyDateProvider);
    final walletCategoryDataAsync = ref.watch(
    ...
    return walletCategoryDataAsync.when(
      ...
      data: (data) {
        if (data.isEmpty) {
          return const Center(
            child: Text(
              'Add a category with a wallet to see your daily gauges.',
            ),
          );
        }
...
class _SpendingGaugeState extends ConsumerState<_SpendingGauge>
  ...
  double _calculateProgress() {
    ...
    return recommendedSpend > 0
        ? widget.data.spendingToday / recommendedSpend
        : 0.0;
  }
