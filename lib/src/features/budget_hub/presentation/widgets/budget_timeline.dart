...
class _BudgetTimelineState extends State<BudgetTimeline> {
  ...
  void _handleTap(Offset localPosition) {
    ...
    final int tappedIndex = ((localPosition.dx + scrollOffset) / itemWidth)
        .floor();
...
class TimelinePainter extends CustomPainter {
  ...
  List<double> _calculateGradients(List<Offset> points) {
    ...
    gradients.add(
      (points[lastIndex].dy - points[lastIndex - 1].dy) /
          (points[lastIndex].dx - points[lastIndex - 1].dx),
    );
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxAmount = spendingData.isNotEmpty
        ? spendingData.map((d) => d.amount).reduce((a, b) => a > b ? a : b)
        : 0.0;
    final minAmount = spendingData.isNotEmpty
        ? spendingData.map((d) => d.amount).reduce((a, b) => a < b ? a : b)
        : 0.0;
    final double range = (maxAmount - minAmount).abs() < 1
        ? 1.0
        : (maxAmount - minAmount);
    ...
    for (int i = 0; i < spendingData.length; i++) {
      ...
      final y =
          topPadding +
          chartHeight -
          ((item.amount - minAmount) / range * chartHeight);
      points.add(Offset(x, y));

      final trendY =
          topPadding +
          chartHeight -
          ((movingAverage[i] - minAmount) / range * chartHeight);
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      _drawText(
        ...
        point + Offset(0, verticalOffset),
        isSelected: isSelected,
      );
      _drawText(
        canvas,
        DateFormat.MMM().format(item.date),
        Offset(point.dx, size.height - 15),
        isSelected: isSelected,
      );
    }
  }
